...
from .forms import (
    ...
    ResetPasswordForm,
    UpdateUserModelForm,
    ImportSubUserDateForManagerForm,
)
from utils.import_date.import_excel import import_data
from utils.import_date.import_subs import import_subs_for_manager
...
class ImportSunUserDateForManagerView(LoginRequiredMixin, TemplateView):
    template_name = "account_module/import_subuser_date_for_manager.html"

    def get_context_data(self, **kwargs):

        context = TemplateLayout.init(self, super().get_context_data(**kwargs))

        form = ImportSubUserDateForManagerForm()
    ...
    def post(self, request: HttpRequest):
        form = ImportSubUserDateForManagerForm(request.POST, request.FILES)
        ...
        if form.is_valid():
            ...
            try:
                ...
                file.seek(0)  # برای استفاده‌های بعدی
                check_file = True
            except Exception:
                check_file = False
            ...
            if check_file:
                ...
                if file.name == "demo.xlsx":

                    manager = request.user
                    results = import_subs_for_manager(manager, file)

                    if len(results["errors"]) < 1:
                        messages.success(request, "زیرمجموعه ها اضافه شدند")

                        return redirect("sub_users")
                    else:
                        form.add_error(
                            "excel_file", f"{len(results['errors'])} خطا یافت شد"
                        )
                        for error in results["errors"]:
                            form.add_error("excel_file", str(error))

                        if "success_add" in results.keys():
                            messages.success(
                                request, f"{results['success_add']} زائر اضافه شد"
                            )

                        return self.render_to_response(
                            self.get_context_data(
                                form=form, errors=form.errors["excel_file"]
                            )
                        )
                else:
                    form.add_error("excel_file", "دقیقا باید فایل نمونه آپلود بشه")
                    return self.render_to_response(
                        self.get_context_data(
                            form=form, errors=form.errors["excel_file"]
                        )
            ...
            form.add_error("excel_file", "فایل اکسل معتبر نیست یا فرمتش اشتباه است.")

        return self.render_to_response(
            self.get_context_data(form=form, errors=form.errors["excel_file"])
        )
