...
class CreateInsurance(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if SettingSite.objects.exists():
            context["start_date"] = (
                SettingSite.objects.first().insurance_start_date.strftime("%Y-%m-%d")
            )
            context["end_date"] = (
                SettingSite.objects.first().insurance_end_date.strftime("%Y-%m-%d")
            )
...
class GetAvailabelSubs(LoginRequiredMixin, TemplateView):
    template_name = "insurance_module/available_insurance_subs.html"
...
class InquiryInsuranceView(View):
    def get(self, request):
        form = InquiryInsuranceForm()
        return render(
            request, "insurance_module/inquiry_insurance.html", {"form": form}
        )

    def post(self, request):
        form = InquiryInsuranceForm(request.POST)
        if form.is_valid():
            code = form.cleaned_data.get("insurance_code")
            get_insurance = Insurance.objects.filter(code=code).first()
            if get_insurance:
                return redirect("inquiry_results", code)
            else:
                form.add_error("insurance_code", "بیمه نامه یافت نشد")
...
class InquiryResults(TemplateView):
    template_name = "insurance_module/inquiry_results.html"
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if check_nationalCode:
                ...
                messages.success(request, "زائر ایجاد شد")
...
def remove_subuser(request: HttpRequest, id):
    ...
    if check_user_and_usersub:
        ...
        if not check_insurance:
            SubUser.delete(sub)
            messages.success(request, "زائر حذف شد")
            return redirect("sub_users")
        else:
...
def generate_pdf(request, code):
    ...
    if insurance.insurance_name:
        buffer = generate_pdf_file(
            code, sub_users, insurance.insurance_name, insurance.manager
        )
    else:
        buffer = generate_pdf_file(code, sub_users, manager=insurance.manager)
